...
(async () => {
  ...
  let bitmapType: ImageBitmap = await createImageBitmap(new Blob());
  ...
  image = bitmapType;
  image = {src: ''};
  image = {src: bitmapType};
  image = {src: blob};
  stringType = Image.from(image).src as string;
  bitmapType = Image.from(image).src as ImageBitmap;
  blob = Image.from(image).src as Blob;
  image = '';
  image = bitmapType;
  numberOrUndefinedOrAuto = Image.from(image).width;
  numberOrUndefinedOrAuto = Image.from(image).height;
  numberOrUndefinedOrAuto = Image.from(image).scale;
